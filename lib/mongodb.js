...
if (!process.env.MONGODB_URI2) {
  console.warn('CẢNH BÁO: Không tìm thấy MONGODB_URI2 trong biến môi trường, ứng dụng sẽ chạy ở chế độ demo');
}
...
export const connectDB = async () => {
  // Kiểm tra kết nối hiện có trước bằng readyState
  ...
  try {
    ...
    if (!uri) {
      throw new Error('MONGODB_URI2 không được cấu hình trong biến môi trường');
    }
